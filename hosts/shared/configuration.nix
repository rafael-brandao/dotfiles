...
in {
  config =
    ...
    (
      [
        {
          ...
          home-manager = {
            ...
            extraSpecialArgs = {
              ...
              hostcfg = removeAttrs hostcfg ["path" "users"];
            };
          ...
          networking = mkMerge [
            {
              ...
              useDHCP = mkDefault (hostcfg.hasLabel "vm");
            }
            (mkIf (!config.networking.wireless.enable) {
              networkmanager = {
                enable = mkDefault true;
                enableStrongSwan = mkDefault true;
              };
            })
          ];
        ...
        (mkIf config.sops.enable {
          sops = {
            ...
            defaultSopsFile = mkDefault (hostcfg.path + /secrets.yaml);
            gnupg.sshKeyPaths = [];
            validateSopsFiles = mkDefault true;
          };
        })

        (mkIf (! hostcfg.hasAnyLabelIn ["wsl"]) {
          boot = {
            kernelPackages = mkIf (!hostcfg.hasLabel "iso") (mkOverride 500 defaults.kernelPackage);
          ...
          services = {
            kmscon = {
              ...
              # hwRender = mkDefault config.hardware.graphics.enable; # Makes infinite recursion error
              fonts = mkDefault [
                {
      ...
      ++ (forEach (attrValues hostcfg.users) (
        usercfg: let
          ...
          homeUsercfg = mkMerge (outputs.lib.local.getUserHomeModules usercfg);
        in
          mkMerge [
            {
